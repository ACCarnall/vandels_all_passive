...
def load_vandels_refit(ID):
    spectrum, photometry = load_vandels(ID)

    if ID == "UDS-GROUND199793SELECT":
        spectrum[(spectrum[:, 0] > 8800.), 2] = 9.9*10**99

    if ID == "CDFS-GROUND105200SELECT":
        spectrum[(spectrum[:, 0] < 5600.), 2] = 9.9*10**99

    if ID == "CDFS-GROUND104129SELECT":
        photometry[5, 2] = 9.9*10**99
...
refit_IDs = ["CDFS-GROUND105200SELECT", "CDFS-GROUND104129SELECT", "UDS-GROUND199793SELECT"]

mask = np.isin(IDs, refit_IDs)
...
fit_cat = pipes.fit_catalogue(IDs, fit_instructions, load_vandels, run="v14_refit",
